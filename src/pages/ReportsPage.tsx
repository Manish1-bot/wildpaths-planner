...
export default function ReportsPage() {
  ...
  const { analyses } = useAnalysis(selectedProjectId);
  ...
  const generatePDF = async () => {
    if (!selectedProject || !latestAnalysis) return;
    ...
    try {
      ...
      if (selectedProject.region) {
        doc.setFontSize(11);
        doc.text(`Region: ${selectedProject.region}`, 20, y);
        y += 6;
      }
      ...
      // Save
      doc.save(`${selectedProject.name.replace(/\s+/g, '_')}_Report.pdf`);
      setGenerated(true);
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="mb-8">
          <h1 className="text-3xl font-heading font-bold mb-2">Reports</h1>
          <p className="text-muted-foreground">
            Generate comprehensive PDF reports for your analysis
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Card className="glass-card">
            <CardHeader>
              ...
              <CardDescription>
                Select a project with completed analysis to generate a PDF report
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              ...
              {selectedProjectId && !latestAnalysis && (
              ...
              <Button
                onClick={generatePDF}
                disabled={!latestAnalysis || generating}
                className="w-full"
              >
                {generating ? (
                  <>
